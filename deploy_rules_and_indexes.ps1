...
Write-Host "`nDeploying Firestore Security Rules..." -ForegroundColor Cyan
...
Write-Host "`nDeploying Firestore Indexes..." -ForegroundColor Cyan
firebase deploy --only firestore:indexes

if ($LASTEXITCODE -eq 0) {
    Write-Host "✓ Firestore indexes deployed successfully!" -ForegroundColor Green
    Write-Host "`nNote: Index creation can take several minutes. Monitor progress at:" -ForegroundColor Yellow
